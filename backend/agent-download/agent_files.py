...
# pysnmp-lextudio — современный форк pysnmp (работает на Python 3.10/3.11/3.12)
...
if errorlevel 1 (
    ...
    echo Скачай Python 3.10+ с https://python.org и при установке поставь галку "Add Python to PATH".
    pause
    exit /b 1
)

echo [1/4] Версия Python:
python --version
echo.
...
echo [3/4] Удаляю старый pysnmp (если есть)...
python -m pip uninstall -y pysnmp pysnmp-lextudio 2>nul
echo.

echo [4/4] Ставлю pysnmp-lextudio + paramiko...
python -m pip install --upgrade pysnmp-lextudio pyasn1 paramiko
if errorlevel 1 (
    echo.
    echo [ОШИБКА] Не удалось поставить пакеты.
    echo Проверь интернет и права доступа. Можно запустить как администратор.
    pause
    exit /b 1
)
...
echo ============================================================
python -c "from pysnmp.hlapi import getCmd; print('  [OK] pysnmp работает')" 2>nul || echo   [!] pysnmp не загрузился
python -c "import paramiko; print('  [OK] paramiko работает')" 2>nul || echo   [!] paramiko не загрузился
echo.
