...
echo [4/5] Ставлю pysnmp (pure-python, без cryptography)...
...
python -m pip install --upgrade pysmi==0.3.4 --no-deps 2>nul
echo.

echo [5/5] Ставлю paramiko без cryptography (для SSH с логином/паролем)...
python -m pip install --upgrade bcrypt --only-binary=:all: 2>nul
python -m pip install --upgrade pynacl --only-binary=:all: 2>nul
python -m pip install --upgrade --no-deps paramiko
if errorlevel 1 (
    echo [WARN] paramiko не поставился — SSH работать не будет, но Telnet и SNMP останутся.
)
...
python -c "import paramiko; print('  [OK] paramiko работает (SSH)')" 2>nul || echo   [!] paramiko не загрузился (Telnet всё равно работает)
python -c "import telnetlib; print('  [OK] telnet работает')" 2>nul || echo   [!] telnetlib не доступен
echo.
